...
void siralama(int dizi[],int sayi) /* bubble sort siralamasý ile notlarý sýralama */
	{	
	int temp;
...
int medyanBulma(int dizi[],int sayi)
	...
	Medyan=(dizi[medyanTemp]+dizi[medyanTemp+1])/2; /* eger ogrenci sayýsý tekse ortadaki sayýlarý toplayýp 2 ye bölere medyaný bulma */
	return Medyan;
...
int main()
{
	...
	for (int i=0; i<ogrenciSayisi; i++)
	{
		Sonuc[i] = (Sonuc[i] * 100) / maxNot; /* soru sayýsýna gore puaný 100 luk sisteme cevirme */
	}	
	siralama(Sonuc,ogrenciSayisi);
	max=Sonuc[ogrenciSayisi-1]; /* dizi artýk sýralý olduðu için dizinin son elemaný max ilk elamaný min ( dizi 0 dan basladigindan toplam elaman sayisindan -1 cýkarýrsan son elamaný buluruz */
	min=Sonuc[0];
	aciklik=max-min;
	Dosya.close();
	for(int i = (ogrenciSayisi-1); i>=0; i--) /* odevde istenildiði gibi max tan mine sýralama */
	{
		Dosya2 << ogrenciler[i][0] << ","; /* dizidikeri dosyaya yazdirma */
		Dosya2 << Sonuc[i] << endl;
	}
	Dosya2 << "En yuksek not= "<< max << " En dusuk not= " << min << " Sýnýf ortalamasý= " << ortBulma(Sonuc,ogrenciSayisi) << " Sýnýf medyaný= " << medyanBulma(Sonuc,ogrenciSayisi) << " Sýnýfýn acýklýgý= " << aciklik;	
	cout <<endl << "Tamamlandi." << endl;	
	Dosya2.close();	
	return 0;
	/* c++ ý iyi bilmedigim icin c gibi yazýp sonra c++ a cevirdim o yüzden kod karmasýkligi olabilir */
